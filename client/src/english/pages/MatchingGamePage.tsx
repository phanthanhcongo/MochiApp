...
const MatchingGamePage: React.FC = () => {
  ...
  const adjustReviewTimeOnServer = useCallback((wordId: number, isCorrect: boolean) => {
    ...
    const storageKey = 'matching_reviewed_words_english';
    ...
    fetch(`${getApiUrl()}/en/practice/adjust-review/${wordId}`, {
      method: 'POST',
      headers: {
  ...
  useEffect(() => {
    ...
    if (leftoverQueue.length > 0) {
      console.log(`🔄 Phát hiện ${leftoverQueue.length} log ghép thẻ tiếng Anh tồn đọng, đang đồng bộ...`);
      
      leftoverQueue.forEach((item) => {
        fetch(`${getApiUrl()}/en/practice/adjust-review/${item.wordId}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Accept': 'application/json',
  ...
  const handleStartGame = () => {
    ...
    fetch(`${getApiUrl()}/en/practice/random?limit=${randomLimit}${levelQuery}`, {
      method: 'GET',
      headers: {
        'Accept': 'application/json',
        'Authorization': `Bearer ${token}`,
      ...
      .then((data) => {
        ...
        const wordsList: VocabularyWord[] = scenarios.map((s: any) => ({
          id: s.word.id,
          word: s.word.word,
  ...
  useEffect(() => {
    ...
    // Generate keys (English words) and shuffle
      ...
      <div className="flex-1 flex flex-col justify-start items-center relative overflow-hidden bg-gradient-to-tr from-indigo-50/40 via-slate-50 to-emerald-50/30 py-8 px-4">
        ...
        {gameState === 'select' && (
          ...
          >
            ...
            <h2 className="text-2xl font-black text-slate-800 tracking-tight text-center">Game Ghép Thẻ Từ Vựng</h2>
            <p className="text-slate-500 text-xs sm:text-sm text-center mt-2 leading-relaxed">
              Trò chơi rèn luyện trí nhớ! Ghép các từ tiếng Anh với nghĩa tiếng Việt tương ứng. Trò chơi này hoạt động độc lập và không ảnh hưởng đến lịch sử học tập của bạn.
            </p>
        ...
        {gameState === 'playing' && (
          <div className="w-full max-w-4xl flex flex-col items-center">
            ...
            <div className="text-center mb-6">
              ...
              <p className="text-slate-500 text-xs sm:text-sm mt-1">Ghép các từ vựng tiếng Anh bên trái với nghĩa tương ứng bên phải.</p>
            </div>

            {/* Columns Container */}
            <div className="grid grid-cols-2 gap-4 sm:gap-6 w-full max-w-3xl mb-8">

              {/* LEFT COLUMN: Vocabulary Keys */}
                  ...
                  {keys.map((card) => {
                    ...
                    return (
                      <motion.button
                        ...
                        className={`h-16 sm:h-20 w-full px-4 rounded-2xl border text-center font-bold text-sm sm:text-base md:text-lg flex items-center justify-center transition-all duration-300 cursor-pointer ${cardStyle}`}
                        style={{ willChange: 'transform' }}
                      >
                        <span className="truncate max-w-full flex items-center justify-center gap-1 sm:gap-2">
                          {card.text}
                          {!isMatched && <FaVolumeUp className="text-[10px] sm:text-xs text-slate-400/80 hover:text-indigo-500 inline-block" />}
                        </span>
                      </motion.button>
                    );
                  })}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[220px] overflow-y-auto pr-1">
                {allWords.map((item) => (
                  <div key={item.id} className="bg-slate-50 border border-slate-100 rounded-xl px-4 py-2.5 flex items-center justify-between text-xs sm:text-sm">
                    ...
                    <span className="text-slate-500 font-semibold">{item.meaning_vi}</span>
                  </div>
                ))}
              </div>
            </div>
